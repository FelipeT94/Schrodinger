...
class Solver():
    ...
    Atrributes:
    """
    def __init__(self, V, e, ni=100, de=0.1, e_tol=1e-6):
        self.parse_V(V)
    ...
    def secant(self, n, dt, x, dx):
        ...
        while ((abs(dx)>dt) and (k<n)):
            ...
            x, x1 = x1, x2
            if (abs(x1-x)>delta):
              x1=np.sign(x1-x)*delta+x #don't know
            dx = x1-x
            k += 1
            if (k==n):
    ...
    def f(self, x):
        ...
        """
        nl, nr = self.wave(x)
        if max([nr, nl]) > self.nx: #don't know
            return 0
        else:
...
if __name__ == "__main__":
    eigs, ns, waves, potentials = [], [], [], []
    M = 0
    for i in range(0,400):
        '''
        params passes along the information needed to tell the program
        the type of function we are looking for as well as the 
        paramaters, alpha, lambda, omega, and m.
        '''
        params = {'omega': 1.0, 'm': M, 'func': 'oscilator'}
        M +=.1
        vs = potential(**params).data # call the class "potential" to obtain the potential array
    
        minv = np.min(vs[:, 1]) #minimum potential
        maxv = np.max(vs[:, 1])-.1 # max potential, don't know why its minus .1
        #print("Level    init_value   Eigenvalue")
        '''
        "e" = initail energy value used as guesses
        expand range in order to find more eigenstates
        refine spacing if needed
        '''
        for e in np.linspace(minv, maxv,10): #range in which were finding the values of e in
            solver = Solver(vs, e)
            #print("{:4d} {:12.4f} {:12.5f}".format(solver.n, e, solver.eigenvalue))
            if solver.valid and solver.n == 0:
                ns.append(solver.n)
                eigs.append(solver.eigenvalue)
                waves.append(solver.u)
                potentials.append(vs[:,1])
                break
    data = []
    data.append(vs[:,0])
    data.append(eigs)
    data.append(waves)
    data.append(potentials)
    import pickle
    with open('waves.data','wb') as filehandle:
        pickle.dump(waves,filehandle)
    with open('potentials.data','wb') as filehandle:
        pickle.dump(potentials,filehandle)
    with open('eigenvalues.data','wb') as filehandle:
        pickle.dump(eigs,filehandle)

                    
    
    #np.savetxt('training_u.txt',waves)
    #np.savetxt('training_v.txt',potentials)
    #np.savetxt('training_e.txt',eigs)
    #np.savetxt('training_x.txt',vs[:,0])
