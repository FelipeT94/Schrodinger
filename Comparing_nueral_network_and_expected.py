...
if __name__ == "__main__":
    omega_1 = 1
    omega_2 = 5
    v0 = potential(omega = omega_1, m = m).data
    v = process_potential(v0)
    v0_2 = potential(omega = omega_2, m = m).data
    v_2 = process_potential(v0_2)
    ...
    X1.append(v[:,1])
    X1.append(v_2[:,1])
    y1.append(solver.eigenvalue)
    y1.append(solver2.eigenvalue)
    ...
    # Fitting
    clf = MLPRegressor(hidden_layer_sizes=(120,3), max_iter=1500, random_state=1)
    clf.fit(X_0, y)
    w = []
    w.append(omega_1)
    w.append(omega_2)
